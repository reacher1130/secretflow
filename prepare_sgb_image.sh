...
OPERATOR_IMAGE="secretflow-registry.cn-hangzhou.cr.aliyuncs.com/secretflow/sgb"

usage="$(basename "$0") [OPTIONS]

OPTIONS:
    -h    show this help text
    -i    (optional) operator image info, default value: secretflow-registry.cn-hangzhou.cr.aliyuncs.com/secretflow/sgb
    -k    (optional) kuscia container name, default value: ${USER}-kuscia-autonomy-alice

example:
 ./bfia/prepare_sgb_image.sh -k ${USER}-kuscia-autonomy-alice -i secretflow-registry.cn-hangzhou.cr.aliyuncs.com/secretflow/sgb
"
...
function prepare_kuscia_app_image() {
  local image_name=$1
  local image_tag=$2
  app_image_name="/tmp/sgb-appImage.yaml"
  local image_id=$(docker inspect --format='{{.Id}}' "$image_name:$image_tag" 2>/dev/null)
...
docker exec -it ${KUSCIA_CONTAINER_NAME} kubectl annotate appimage sgb kuscia.secretflow/component-spec='{"component.1.description":"sgb","component.1.input.1.categories":"dataset","component.1.input.1.description":"train data","component.1.input.1.name":"train_data","component.1.name":"sgb","component.1.output.1.categories":"dataset","component.1.output.1.description":"train data","component.1.output.1.name":"train_data"}' --overwrite

rm -rf ${app_image_name}
echo -e "${GREEN}Finished preparing appImage '${OPERATOR_IMAGE}' in container '${KUSCIA_CONTAINER_NAME}'${NC}"
}

function prepare_bfia_sgb_image() {
