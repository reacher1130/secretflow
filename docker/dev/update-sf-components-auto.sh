...
OPTIONS:
	...
	-p    [optional] the party name of deploying the secretpad and kuscia container, default value: ${PARTY}
example:
 ./update-sf-components.sh -u ${USER} -p ${PARTY} -i secretflow-registry.cn-hangzhou.cr.aliyuncs.com/secretflow/secretflow-lite-anolis8:latest
"

while getopts ':hi:u:p:' option; do
	case "$option" in
	h)
		echo "$usage"
		exit
...
SECRETPAD_CONTAINER_NAME="${DEPLOY_USER}-kuscia-autonomy-secretpad-${DEPLOY_PARTY}"
